...
7. Set `completedAt:YYYY-MM-DD` when transitioning to done
8. **Cascade to children**: Find all descendant tasks and mark them as done too
9. Write file back
10. Re-run indexer to update `todos.json` (unless `--no-reindex`)
11. **Auto-sync**: Run `tmd sync` on configured files (unless `--no-sync`)
