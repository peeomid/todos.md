...
* **Index/cache**: `tasks.json` (or similar) generated by `tmd index`.
 ...
 * Run `tmd index` again to resync `tasks.json` with Markdown.
 * Exit to shell.
...
* `r` – refresh (re-read `tasks.json` without restarting; optional v2).
...
* `show_done` is not part of the query string itself.
* Filter logic applies it on top of the parsed query.
...
* All edits **must** be applied to Markdown, not `tasks.json`.
 ...
 * Run `tmd index` again to regenerate `tasks.json` with all changes.

### 9.3. Task-to-file mapping

Index (`tasks.json`) must contain enough info to locate tasks:
...
* The TUI locates the task by `(file_path, project_id, local_id)` and rewrites the line in that file (re-parsing that file if needed).
 ...
 * Re-parse that file, update in-memory tasks from it.
