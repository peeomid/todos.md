/**
 * Filter functions for the list command
 */

import type { Task, Energy, Priority } from '../schema/index.js';
import { parseDateSpec, isDateInRange, isOverdue } from './date-utils.js';
 ...
 * Sort tasks by field
 */
export type SortField = 'due' | 'created' | 'project' | 'energy' | 'priority' | 'bucket';
...
export function sortTasks(tasks: Task[], sortBy: SortField): Task[] {
  return [...tasks].sort((a, b) => {
    switch (sortBy) {
      case 'due': {
        // Tasks with due dates come first, then sort by date
      ...
      case 'project': {
        ...
        // Within same project, sort by local ID
